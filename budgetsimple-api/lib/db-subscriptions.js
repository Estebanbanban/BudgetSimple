...
async function getTransactionsForRange(fastify, userId, startDate, endDate) {
  ...
  try {
    fastify.log.info(
      `Fetching transactions for userId: ${userId}, date range: ${startDate} to ${endDate}`
    );

    // First, check total transactions for this user (for debugging)
    ...
    fastify.log.info(
      `Total transactions for user ${userId}: ${totalCount || 0}`
    );
    ...
    let query = fastify.supabase
      .from("transactions")
      .select(
        "id, date, merchant, description, amount, currency, category_id, category_name, type"
      )
      .eq("user_id", userId)
      .gte("date", startDate)
      .lte("date", endDate);
    ...
    const { data: allData, error: allError } = await query.order("date", {
      ascending: true,
    });

    fastify.log.info(
      `Transactions in date range (before filtering): ${allData?.length || 0}`
    );

    if (allError) {
      fastify.log.error(
        allError,
        "Error fetching transactions (with type field)"
      );
      // Try without type filter
      const { data: dataNoType, error: errorNoType } = await fastify.supabase
        .from("transactions")
        .select(
          "id, date, merchant, description, amount, currency, category_id, category_name"
        )
        .eq("user_id", userId)
        .gte("date", startDate)
        .lte("date", endDate)
      ...
      if (errorNoType) {
        fastify.log.error(
          errorNoType,
          "Error fetching transactions (without type field)"
        );
        // Try to get any transactions for this user (for debugging)
        const { data: anyData } = await fastify.supabase
          .from("transactions")
          .select("user_id, date")
          .eq("user_id", userId)
          .limit(5);
        fastify.log.info(
          `Sample user_ids in transactions table: ${JSON.stringify(anyData)}`
        );
        return [];
      }
      ...
      fastify.log.info(
        `Fetched ${expenses.length} expense transactions (filtered by negative amount) for subscription detection (${startDate} to ${endDate})`
      );
      if (expenses.length === 0 && dataNoType && dataNoType.length > 0) {
        fastify.log.warn(
          `Found ${
            dataNoType.length
          } transactions but none are expenses (all amounts are positive). Sample: ${JSON.stringify(
            dataNoType.slice(0, 2)
          )}`
        );
      }
      return expenses;
    ...
    if (hasTypeField) {
      ...
      fastify.log.info(
        `Fetched ${expenses.length} expense transactions (filtered by type='expense') for subscription detection (${startDate} to ${endDate})`
      );
      if (expenses.length === 0 && allData && allData.length > 0) {
        const types = [...new Set(allData.map((tx) => tx.type))];
        fastify.log.warn(
          `Found ${
            allData.length
          } transactions but none have type='expense'. Available types: ${JSON.stringify(
            types
          )}. Sample: ${JSON.stringify(allData.slice(0, 2))}`
        );
      }
    } else {
      ...
      if (expenses.length === 0 && allData && allData.length > 0) {
        const amounts = allData.map((tx) => parseFloat(tx.amount || 0));
        fastify.log.warn(
          `Found ${
            allData.length
          } transactions but none are expenses (all amounts are positive or zero). Amount range: ${Math.min(
            ...amounts
          )} to ${Math.max(...amounts)}. Sample: ${JSON.stringify(
            allData.slice(0, 2)
          )}`
        );
      }
...
async function getSubscriptionCandidates(fastify, userId, status) {
  ...
  try {
    ...
    const { data, error } = await query.order("confidence_score", {
      ascending: false,
    });
...
async function getSubscriptionCandidateDetails(fastify, userId, candidateId) {
  ...
  try {
    ...
    return {
      ...subscription,
      contributingTransactions: (transactions || [])
    ...
    fastify.log.error(
      error,
      "Exception fetching subscription candidate details"
    );
    return null;
  }
}
...
async function storeSubscriptionCandidates(fastify, userId, candidates) {
  ...
  try {
    ...
    if (mappings.length > 0) {
      await fastify.supabase.from("subscription_transactions").insert(mappings);
    }
...
async function getSubscriptionSummary(fastify, userId) {
  ...
  try {
    ...
    const totalMonthly = subscriptions.reduce(
      (sum, s) => sum + parseFloat(s.estimated_monthly_amount || 0),
      0
    );
...
async function getSubscriptionTransactions(
  ...
  try {
    ...
    const { data, error } = await query.order("transactions(date)", {
      ascending: false,
    });
